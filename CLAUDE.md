...
**Off-org PRs are force-created as drafts — this is expected, not an error.** `gh pr create` targeting a repo whose owner is not `smartwatermelon` or `nightowlstudiollc` is hard-forced to `--draft` by `gh-wrapper.sh`, mechanically — no opt-out, no env var escape hatch (`smartwatermelon/dotfiles#174`/`#175`). If a PR you just opened comes back as a draft and you didn't ask for that, this is why. Do not treat it as a bug and do not work around it: not via `gh pr edit --ready`/`gh pr ready`, not via the REST/GraphQL API, not by locating and invoking the bare `gh` binary directly (e.g. `/opt/homebrew/bin/gh`) to bypass the wrapper. Surface the draft PR to the user as usual (repo, number, URL, one line on what it addresses) and note it's a draft pending the human's discretion — only the human promotes it out of draft, via the GitHub UI.
